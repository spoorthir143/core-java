class PrintedAnarkali{


				static		String name = "PrintedAnarkaliKurta";
				static		String material = "compositionViscose";
				static		String length = "Calf Length";
				static		String sleeveType = "3/4 Sleeve";
			
				
				static		String neckStyle = "V-Neck";
				static		String style = "Anarkali";
				static		String itemWeight = "300 g";
				static		String department = "Women";
				static		int netQuanity = 1;
				static		String includedComponents = "Dupatta, Kurta, Pants";



					public static void main(String[] PrintedAnarkaliKurta){

					//String name = "PrintedAnarkaliKurta";
				//	String material = "compositionViscose";
				//	String length = "Calf Length";
				//	String sleeveType = "3/4 Sleeve";
				//	String neckStyle = "V-Neck";
				//	String style = "Anarkali";
				//	String itemWeight = "300 g";
				//	String department = "Women";
				//	int netQuanity = 1;
				//	String includedComponents = "Dupatta, Kurta, Pants";

System.out.println("name is "+name);
System.out.println("material is "+material);
System.out.println("length is "+length);
System.out.println("sleeveType is "+sleeveType);
System.out.println("neckStyle is "+neckStyle);
System.out.println("style is "+style);
System.out.println("itemWeight is "+itemWeight);
System.out.println("department is "+department);
System.out.println("netQuanity is "+netQuanity);
System.out.println("includedComponents is "+includedComponents);
}
}